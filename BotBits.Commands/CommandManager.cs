...
namespace BotBits.Commands
{
    ...
    public sealed class CommandManager : EventListenerPackage<CommandManager>, IEnumerable<Command>, IDisposable
    {
        ...
        public Command this[string name]
        {
            ...
            {
                Command cmd;
                if (!this.TryGetCommand(name, out cmd)) throw new InvalidOperationException("Unknown command.");
                return cmd;
            }
        }
        ...
        {
            return new Thread(() => { while (!cancellationToken.IsCancellationRequested) this.ReadNextConsoleCommand(); }) { IsBackground = true, Name = "BotBits.Commands.CommandReader" };
        }
